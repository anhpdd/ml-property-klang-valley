...
class OSMCache:
    ...
    def get(self, osm_type: str, osm_id: str) -> Optional[ET.Element]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to load cache for {osm_type}/{osm_id}: {e}")
            return None
...
def extract_geometries_for_ways(
    ...
    logger.info(
        f"✅ Extracted {success_count:,}/{len(df):,} geometries ({success_rate:.1f}%)")
...
def fetch_relation_geometry(
    ...
    if not polygons:
        logger.warning(
            f"Could not form valid polygons for relation {relation_id}")
        return None
    ...
    logger.info(
        f"✓ Processed relation {relation_id} with {len(polygons)} polygon(s)")
...
def create_district_geodataframe(
    ...
    """
    logger.info(
        f"📍 Fetching {len(district_dict)} district boundaries from OSM...")
...
def validate_road_locations(
    ...
    if roads_gdf.crs != districts_gdf.crs:
        logger.info(
            f"Reprojecting roads to match districts CRS: {districts_gdf.crs}")
    ...
    for idx, road in tqdm(roads_gdf.iterrows(), total=len(roads_gdf), desc="Validating"):
        ...
        record['is_in_correct_district'] = (
            assigned_district == best_district) if best_district else False
        record['overlap_length'] = max_overlap
        record['total_road_length'] = road_geom.length
        record['overlap_percentage'] = (
            max_overlap / road_geom.length * 100) if road_geom.length > 0 else 0
    ...
    logger.info(
        f"   • Correctly located: {correct_count:,}/{len(validated_gdf):,} ({correct_pct:.1f}%)")
...
def run_spatial_validation_pipeline(
    ...
    logger.info(
        f"   • Records with geometry: {validated_df['overlap_length'].notna().sum():,}")
    logger.info(
        f"   • Correctly located: {validated_df['is_in_correct_district'].sum():,}")
    logger.info("="*60)
