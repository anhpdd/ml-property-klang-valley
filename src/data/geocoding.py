...
def setup_geocoder(config: GeocodingConfig) -> Tuple[Nominatim, RateLimiter]:
    ...
    logger.info(f"✓ Geocoder initialized with {config.min_delay_seconds}s rate limit")
...
def load_checkpoint(checkpoint_file: Path) -> Dict:
    ...
    if checkpoint_file.exists():
        with open(checkpoint_file, 'r') as f:
            data = json.load(f)
        logger.info(f"✓ Loaded checkpoint with {len(data['road_to_way_id'])} cached roads")
        return data
...
def geocode_unique_roads(
    ...
    logger.info(f"   • Already cached: {len(unique_roads) - len(roads_to_process):,}")
    ...
    for idx, road_name in enumerate(tqdm(roads_to_process, desc="Geocoding")):
        way_id, node_info = geocode_single_road(road_name, geocode_func, config)
...
def save_geocoding_results(
    ...
    """
    if columns is None:
        columns = ['road_name', 'way_id', 'found_node_id', 'found_node_name', 'district']
    
    unique_results = df[columns].drop_duplicates().reset_index(drop=True)
    unique_results.to_csv(output_file, index=False)
    
    logger.info(f"✅ Saved {len(unique_results):,} unique results to '{output_file.name}'")
...
def run_geocoding_pipeline(
    ...
    logger.info("="*60)
    logger.info(f"   • Way IDs found:  {stats['way_found']:,} ({stats['way_pct']:.1f}%)")
    logger.info(f"   • Node fallbacks: {stats['node_found']:,} ({stats['node_pct']:.1f}%)")
    logger.info(f"   • Not found:      {stats['not_found']:,} ({stats['not_found_pct']:.1f}%)")
    logger.info("="*60)
